...
# Variáveis
access_token = "EAATXaSQjmX8BO5g2dj1bnyZBvpo00ZBeh5W8wZBJrP29J7OBAzZAiaeCORLFs1mxZCfCb6mraCackl7QazH7EGxlfVCwtfIxZCavAZCZCKdcGpgfeT9fNaZCR9UYGZCo7NVtqBt4jCAU1ycwGwxlN2Fa9tGyoBEn5Ml2ZCfCAhK5L4NzomHs5atHZBfyJRBIN5OLQUHSHGWDZCOlLfcEbq5u9asJ3TGZBW5brRsuotifMZD"
phone_number_id = "434398029764267"

# Armazenamento do estado da conversa para cada usuário
user_state = {}

def salvar_estado_usuario(recipient_id, state_data):
    """Salva o estado do usuário no Firestore"""
    doc_ref = db.collection("user_states").document(recipient_id)
    doc_ref.set(state_data)

def carregar_estado_usuario(recipient_id):
    """Carrega o estado do usuário do Firestore"""
    doc_ref = db.collection("user_states").document(recipient_id)
    doc = doc_ref.get()
...
def reply_to_whatsapp_message(event_id, recipient_id, button_payload):
    ...
    if button_payload == "sim":
        ...
        salvar_estado_usuario(recipient_id, user_state)
                            ...
                            for message in change["value"]["messages"]:
                                ...
                                if "button" in message:
                                    ...
                                    if button_payload:
                                        ...
                                        #user_state[sender_id]["state"] = "awaiting_template2_response"
                                        user_state = carregar_estado_usuario(sender_id)
                                        print(f"Estado do usuário: {user_state.get(sender_id)}")
                                        if user_state.get(sender_id, {}).get("state") == "awaiting_template2_response" and button_payload == "Tudo certo!":
                                            ...
                                            #user_state = carregar_estado_usuario(recipient_id)     
