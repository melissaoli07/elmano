...
def reply_to_whatsapp_message(recipient_id, message_text):
    ...
    elif message_text.lower() == "ok":  # Verificar a resposta "ok"
        if user_state.get(recipient_id) == "awaiting_template4_response":
            print(f"Enviando Template 5 para o usuário {recipient_id}")
            template5(recipient_id)
            user_state[recipient_id] = None  # Limpar o estado após enviar o Template 5
        else:
            message_data = {
                "messaging_product": "whatsapp",
                "to": recipient_id,
...
def template5(recipient_id):
    ...
    if response.status_code == 200:
        print("Template 5 enviado com sucesso!")
    else:
        print("Erro ao enviar Template 5:", response.json())
                        ...
                        if "value" in change and "messages" in change["value"]:
                            ...
                            if message.get("button"):
                                ...
                                if user_state.get(sender_id) == "awaiting_template2_response" and button_payload == "Tudo certo!":
                                    ...
                                    # Atualizar o estado após enviar o Template 4
                                    user_state[sender_id] = "awaiting_template4_response"
                                    
                                elif user_state.get(sender_id) == "awaiting_template4_response":
                                    print(f"Enviando Template 5 para o usuário {sender_id}")
                                    template5(sender_id)
                                    
                                    # Limpar o estado após enviar o Template 5
